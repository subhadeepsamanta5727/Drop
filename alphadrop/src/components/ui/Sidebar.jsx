import { ChevronRight, CreditCard, DollarSign, FolderUp, LayoutDashboard, MoreVertical, Users } from 'lucide-react'
...
const adminItems = [
  ...
  { label: 'Users Ledger', to: '/admin/users', icon: Users },
...
export function Sidebar({ mode = 'ADMIN' }) {
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col justify-between overflow-hidden rounded-2xl bg-gradient-to-b from-[#071a37] to-[#0c2d64] p-4 text-white shadow-[0_18px_36px_rgba(7,26,55,0.22)] xl:sticky xl:top-6 xl:h-[calc(100vh-3rem)] xl:w-[252px]">
      <div>
        <div className="mb-5 flex items-center gap-3 px-2 py-2">
          ...
          <button
            ...
            className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-blue-100 transition hover:bg-white/10 xl:hidden"
          >
            <MoreVertical size={20} />
          </button>
        </div>

        <nav className={`${isMenuOpen ? 'grid' : 'hidden'} grid-cols-2 gap-1 xl:block xl:space-y-1`}>
          {items.map(({ label, to, icon: Icon }) => (
            <NavLink
              ...
              className={({ isActive }) => `sidebar-link border border-transparent transition-all duration-200 hover:-translate-x-0.5 hover:border-[#5aa7ff] hover:bg-[#0f6ce5]/35 hover:text-white hover:shadow-[0_8px_18px_rgba(90,167,255,0.18)] ${isActive ? 'active' : ''}`}
            >
              <Icon size={18} />
              <span>{label}</span>
            </NavLink>
      ...
      <div className={`${isMenuOpen ? 'block' : 'hidden'} mt-6 rounded-xl border border-white/10 bg-white/5 p-3 xl:block`}>
        <div className="mb-3 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-400">{profileEmail}</div>
          </div>
        </div>
